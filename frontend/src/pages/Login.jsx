...
function Login() {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      if (response?.requiresTwoFactor) {
        ...
        console.log('📊 [Login] Método 2FA:', response.twoFactorMethod);
        ...
        // Si es Email 2FA, generar y enviar código
        ...
        // Si es TOTP, no hacer nada (el usuario usa su app)
      } else {
  ...
  const handleTwoFactorSubmit = async (e) => {
    ...
    try {
      console.log('🔍 [Login] Verificando código con método:', twoFactorMethod);

      let verifyResponse;

      // Verificar según el método
      if (twoFactorMethod === 'totp') {
        verifyResponse = await totpService.verifyTotp(userId, twoFactorCode);
      } else {
        verifyResponse = await twoFactorService.verifyCode(userId, twoFactorCode);
      }
        ...
        <div className="auth-card card card-elevated">
          {!showTwoFactorForm ? (
            ...
            <>
              ...
              {/* Header */}
              <div className="auth-header">
                <div className="auth-icon">{twoFactorMethod === 'totp' ? '🔑' : '📧'}</div>
